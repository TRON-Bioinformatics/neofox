...
class MixMhc2Pred(AbstractMixMHCpred):
    ...
    def hla_ii_alleles4prediction(self, hla_alleles):
    ...
    def mixmhc2prediction(self, hla_alleles, tmpfasta, outtmp, wt=False):
        ...
        """
        if not wt:
            hla_allele = self.hla_ii_alleles4prediction(hla_alleles)
        elif wt:
    ...
    def import_available_HLAII_alleles(self, path_to_hla_ii_alllele_file):
        """HLA II alleles for which MixMHC2pred predictions are possible"""
        avail_alleles = []
        with open(path_to_hla_ii_alllele_file) as f:
            for line in f:
                line = line.rstrip().lstrip()
                if line:
                    if line.startswith(("L", "A")):
