...
def wrap_pathogensimilarity(props, mhc, fastafile, iedb, affinity = False, nine_mer = False):
    ...
    with open(fastafile , "w") as f:
        ...
        f.write(id + "\n")
        f.write(mhc_mut + "\n")
    pathsim = _calc_pathogensimilarity(fastafile, id, iedb)
...
def calculate_amplitude_mhc(props, mhc, multiple_binding=False, affinity = False, netmhcscore = False, nine_mer= False):
    ...
    try:
        ...
        if nine_mer or affinity:
            amplitude_mhc = str(candidate_amplitude_mhc * (_calculate_correction(score_wild_type)))
        else:
...
def _calculate_correction(score_wild_type):
